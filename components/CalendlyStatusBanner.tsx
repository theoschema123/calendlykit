...
export function CalendlyStatusBanner({ isConnected, userUri, onConnected }: CalendlyStatusBannerProps) {
    ...
    return (
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-emerald-300">Calendly connecte</p>
          {userUri && <p className="text-xs text-emerald-500/80 mt-0.5 truncate max-w-xs">{userUri}</p>}
        </div>
        <button onClick={() => setShowForm(true)} className="text-xs text-emerald-500 hover:text-emerald-300 transition-colors underline">
          Changer de token
        </button>
      </div>
    );
  ...
  return (
    <div className="px-4 py-4 rounded-xl bg-white/[0.04] border border-white/[0.10]">
      <p className="text-sm font-medium text-gray-300 mb-1">Connectez votre Calendly</p>
      <p className="text-xs text-gray-500 mb-3">
        Allez sur{" "}
        <a href="https://calendly.com/integrations/api_webhooks" target="_blank" rel="noopener noreferrer" className="text-indigo-400 underline">
          calendly.com - Integrations - API
        </a>
        {" "}pour creer un Personal Access Token, puis collez-le ici.
      </p>
